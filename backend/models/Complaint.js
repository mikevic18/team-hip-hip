...
class Complaint {
    ...
    static async create(data) {
        ...
        const response = await db.query(
            `INSERT INTO complaints (user_id, title, content, creation_date, update_date) VALUES ($1, $2, $3, $4, $4) RETURNING *;`, [data.user_id, data.title, data.content, timestamp]
        );
    ...
    async updateApproval(data) {
        ...
        const timestamp = now.toISOString().slice(0, 19).replace("T", " ");
        const response = await db.query(
            "UPDATE complaints SET is_approved = $2, update_date = $3 WHERE post_id = $1 RETURNING *;", [this.id, data.is_approved, timestamp]
        );
    ...
    async updateAdminComment(data) {
        ...
        const timestamp = now.toISOString().slice(0, 19).replace("T", " ");
        const response = await db.query(
            "UPDATE complaints SET admin_comment = $2, update_date = $3 WHERE post_id = $1 RETURNING *;", [this.id, data.admin_comment, timestamp]
        );
        if (response.rows.length != 1)
    ...
    async destroy() {
        const response = await db.query(
            "DELETE FROM complaints WHERE complaint_id = $1 RETURNING *;", [this.complaint_id]
        );
        return new Complaint(response.rows[0]);
    }
}
