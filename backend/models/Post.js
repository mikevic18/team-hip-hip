...
class Post {
    ...
    static async getPostsByComplaintID(id) {
        const response = await db.query(
            "SELECT * FROM posts WHERE complaint_id = $1;", [id]
        );
        if (response.rows.length == 0)
            throw new Error("Unable to locate post.");
        return response.rows.map((p) => new Post(p));
    ...
    async update(data) {
        ...
        const timestamp = now.toISOString().slice(0, 19).replace("T", " ");
        const response = await db.query(
            "UPDATE posts SET title = $2, content = $3, update_date = $4 WHERE post_id = $1 RETURNING *;", [this.id, data.title, data.content, timestamp]
        );
        if (response.rows.length != 1) {
    ...
    async updateVotes(data) {
        const response = await db.query(
            "UPDATE posts SET votes = $1 WHERE post_id = $2 RETURNING *;", [this.votes + data.votes, this.id]
        );
        if (response.rows.length != 1)
    ...
    async destroy() {
        const response = await db.query(
            "DELETE FROM posts WHERE post_id = $1 RETURNING *;", [this.id]
        );
        return new Post(response.rows[0]);
    }
}
